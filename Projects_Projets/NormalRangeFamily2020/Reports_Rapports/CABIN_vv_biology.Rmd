...
This report presents the results for verification and validation of a biological CABIN data file associated to the project:
...
```{r}
paste(nrow(dataset.BIO), " visits (lines) and ", ncol(dataset.BIO), " variables (columns).", sep = "")
  # `r {nrow(dataset.BIO)} ` visits (lines) and `r {ncol(dataset.BIO)}` variables (columns).
```
...
```{r 4, echo=FALSE}
#Geographical Distribution of taxa richness
...
box.relf1 <- qplot(y = spe.relf, x = "", geom = "boxplot", ylab = "Occurrence (%)", main = "Distribution of taxa\nrelative frequency") + 
  theme(plot.title = element_text(hjust = 0.5))
box.relf2 <- qplot(y = spe.relf, x = "", geom = "boxplot", ylab = "Log(Occurrence (%))", main = "Distribution of taxa\nrelative frequency", log = "y") + 
  theme(plot.title = element_text(hjust = 0.5))
...
```{r 19, echo=FALSE}
#Correaltion matrix between taxa which frequency > 25%

# Nouvelle méthode
corrgram(Benthos25, lower.panel = panel.pts, upper.panel = panel.conf,
         diag.panel = panel.density, main=paste("Taxa Covariance - for frequency >", FreqMin, "%"))

# Ancienne méthode, jugée trop longue
GGally::ggpairs(Benthos25, progress=TRUE)
for (i in 1:ceiling(length(Benthos25) / 4)) {
  for (j in 1:ceiling(length(Benthos25) / 4)) {
    print(ggduo(Benthos25, columnsX = ((i * 4 - 3):(i * 4))[((i * 4 - 3):(i * 4)) <= length(Benthos25)], columnsY = ((j * 4 - 3):(j * 4))[((j * 4 - 3):(j * 4)) <= length(Benthos25)]))
  }
}
```
...
corrgram(Benthos25, order = FALSE, lower.panel = panel.cor, upper.panel = panel.pie, 
         text.panel = panel.txt, main=paste("Taxa Covariance - for frequency >", FreqMin, "%"))
...
```{r include=FALSE}
render("CABIN_vv_biology.Rmd", c("pdf_document"))
# "html_document", 
```
...
**CABIN_vv_biology.Rmd Version 1.2 — August 13, 2019**
...
![ACTION:](../../../Configuration/logo_new.png)   **First version**.
