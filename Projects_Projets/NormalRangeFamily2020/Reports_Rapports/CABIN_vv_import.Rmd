...
datasetName <- svDialogs::dlgInput("Enter a name for your analysis:", default = "Test", Sys.info()["datasetName"])$res
...
all.dataset.BIO <- read.csv(paste("../Data_Donnees/", bioFilename, sep = ""), as.is=TRUE, header=TRUE,
	 			   na.string=na.vals, sep=SEP, dec=DEC, encoding = "latin1") #, row.names=1
dataset.BIO <- rm.NAcol(all.dataset.BIO)
dataset.BIO <- dataset.BIO[,-187] # ghost column removed from Excel file
...
dataset.SPA <- unique(dataset.SPAb)
...
rm(dataset.SPAb)
paste("A table for spatial data was created from a subset of the files", bioFilename)
...
NoMatch_Dataset <- matrix(ncol=4, nrow=4)
rownames(NoMatch_Dataset) <- c("Biological","General", "Name", "Spatial")
colnames(NoMatch_Dataset) <- c("Biological", "General", "Name", "Spatial")

for(i in 1:nrow(NoMatch_Dataset)){for(j in 1:ncol(NoMatch_Dataset)){NoMatch_Dataset[i,j] <- length(setdiff(get(c("dataset.BIO2", "dataset.GEN", "dataset.NAM", "dataset.SPA")[i])$SampleId, get(c("dataset.BIO2", "dataset.GEN", "dataset.NAM", "dataset.SPA")[j])$SampleId))}}

datatable(NoMatch_Dataset, filter = "top", rownames=T, options = list(scrollX = TRUE ))
```

###Missing Values

Across all four datasets, lists of those samples with missing values are generated for each variable, and saved as external files.
...
|:-----------|:-------:|:------------------------------|:-----------:|:------------------:|<br>
| 2019-08-13 | 2.2     |Code optimization; dialog boxes|             |                    |<br>
| 2019-05-09 | 2.1     |Fixed icon typo                |             |                    |<br>
...
**CABIN_vv_import.Rmd Version 2.2 — August 13, 2019**

![ACTION:](../../../Configuration/logo_updated.png)   **Update** --- Optimization of the code.
![ACTION:](../../../Configuration/logo_new.png)   **New** --- Dialog boxes added.


**CABIN_vv_import.Rmd Version 2.1 - January 16, 2018**
...
Developped by [Martin Jean](mailto:martin.jean@canada.ca), Evelyne Paquette-Boisclair, and Eva Charbonneau-Bérubé
