...
In this analysis, the general dataset is checked to answer the following question:
  ...
  + Are the general data reflecting the sites visited?
...
for (j in 3:ncol(dataset.GENnum)) {
  ...
  dotchart <- ggplot(data = dataset.GENnum, aes(y = seq(dataset.GENnum[,j]), x = dataset.GENnum[,j], na.rm = T)) +
    ...
    xlab("Value") +
    theme(plot.title = element_text(hjust = 0.5)) +
    ggtitle(paste("Cleveland dotplot\n", colnames(dataset.GENnum[j]))) +
...
POlist <- list(ifelse(dataset.GENnum[,j] < quantile(dataset.GENnum[,j], probs=c(.25), na.rm = T) - 1.5*IQR(dataset.GENnum[,j], na.rm = T) | dataset.GENnum[,j] > quantile(dataset.GENnum[,j], probs=c(.75), na.rm = T) + 1.5*IQR(dataset.GENnum[,j], na.rm = T), dataset.GENnum$Site_Date_Number, ""))
print(POlist)
}
...
This version updates the identification of potential outliers by using all numeric variables.
