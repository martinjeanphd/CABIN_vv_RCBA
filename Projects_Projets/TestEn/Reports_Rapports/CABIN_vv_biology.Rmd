...
Afin d'assurer un bon fonctionnement de ce carnet, le code permettant de charger les différents modules doit être exécuté.
...
###Visites présentes dans le fichier
...
###Matrice de corrélation des doubles zéros entre les taxons avec coefficients de corrélation de Pearson - taxons avec fréquence > 25%

La matrice de corrélation suivante illustre la force de corrélation qui existe entre deux taxons présentant des valeurs d'abondances équivalentes à 0 par l'intensité de la couleur. Une corrélation illustrée par la couleur blanche indique une force de corrélation qui tend vers une valeur de 0 et une corrélation illustrée par la couleur bleue foncée indique une force de corrélation qui tend vers une valeur de 1. Les valeurs de corrélation de Pearson entre les taxons y sont indiquées dans la portion inférieure gauche de la figure. Cette matrice illustre également la proportion de valeurs d'abondances à 0 que présente un taxon par rapport à toutes les valeurs d'abondances observées dans ce dernier. Seulement les taxons présents dans plus de 25% des visites y sont représentés. 
...
corrgram(A1, order = FALSE, lower.panel = panel.cor, upper.panel = panel.pie, text.panel = panel.txt, main=paste("Covariance des doubles zéros - taxons avec fréquence >", FreqMin, "%"))
...
Pour chaque variable, le premier graphique (à gauche) illustre par un histogramme la distribution des fréquences des valeurs observées par taxon. Le deuxième graphique (à droite) illustre la distribution des fréquences des valeurs observées par taxon ayant subies une transformation logarithmique. Il permet notamment de vérifier et valider si la distribution des données semble suivre la loi normale. Si la distribution des données sur le deuxième graphique semble ressembler davantage à une distribution normale, une transformation logarithmique des données pourrait être nécessaire. Cet histogramme illustre également l'abondance moyenne par taxon par une ligne pleine ainsi que l'écart-type des abondances par taxon par deux lignes pointillées. 
...
hist.ab.1 <- hist(as.numeric(unlist(dataset.BIO[,i])), breaks = "Sturges", xlab = "Abondance", ylab = "Fréquence", main = paste("Fréquences d'abondances\nde", colnames(dataset.BIO[i])))
xfit <- seq(min(which(!is.na(dataset.BIO[,i]))), max(which(!is.na(dataset.BIO[,i]))), length=norm.n)
yfit_density <- dnorm(xfit, mean = norm.mean, sd=norm.sd)
yfit_freq <- yfit_density*diff(hist.ab.1$mids[1:2])*norm.n
lines(xfit, yfit_freq, col="red", lwd=1)
  abline(v = norm.mean, col = "blue") +
  abline(v = norm.mean + norm.sd, lty = 2, col = "blue") +
  abline(v = norm.mean - norm.sd, lty = 2, col = "blue")

norm.mean.log <- mean(log(which(!is.na(dataset.BIO[,i]))))
norm.sd.log <- sd(log(which(!is.na(dataset.BIO[,i]))))
norm.n.log <- length(log(which(!is.na(dataset.BIO[,i]))))
norm.bin.log <- ceiling(max(log(which(!is.na(dataset.BIO[,i]))))- min(log(which(!is.na(dataset.BIO[,i]))))/nclass.Sturges(log(which(!is.na(dataset.BIO[,i])))))
  
hist.ab.2 <- hist(log(as.numeric(unlist(dataset.BIO[,i]))), breaks = "Sturges", xlab = "Log(Abondance)", ylab = "Fréquence", main = paste("Fréquences d'abondances\nde", colnames(dataset.BIO[i])))
xfit <- seq(min(log(which(!is.na(dataset.BIO[,i])))), max(log(which(!is.na(dataset.BIO[,i])))), length=norm.n.log)
...
####Tests de normalité par variable
...
####Tests de normalité par indice de diversité

Les résultats suivants présentent le résultat obtenu par un test de normalité Shapiro-Wilk appliqué sur chaque indice de diversité calculé à partir du fichier de données. Une valeur de P (p-value) inférieure à 0.05 indique qu'il n'est pas possible de supposer que la distribution des données d'indice de diversité suit la loi normale avec une probabilité de 95%. 

```{r, echo=FALSE, warning=FALSE}
#Tests de normalité Shapiro-Wilk par indice de diversité

print("Occurrences en taxons")
shapiro.test(spe.pres)
print("Fréquences relatives en taxons")
shapiro.test(spe.relf)
print("Richesses en taxons")
shapiro.test(N0)
print("Entropie de Shannon")
shapiro.test(H)
print("Diversité de Shannon (nombre de taxons abondants effectif)")
shapiro.test(N1)
print("Diversité de Simpson (nombre de taxons dominants effectif)")
shapiro.test(N2)
print("Régularité de Pielou")
shapiro.test(J)
print("Indice de régularité de Shannon (Hill's ratio)")
shapiro.test(E10)
print("Indice de régularité de Simpson (Hill's ratio)")
shapiro.test(E20)
```
...
Pour chaque variable, le premier graphique (à gauche) illustre par un histogramme la distribution des fréquences des valeurs calculées d'occurrence en taxons à partir du fichier de données. Le deuxième graphique (à droite) illustre la distribution des fréquences des valeurs calculées d'occurrence en taxons à partir du fichier de données ayant subies une transformation logarithmique. Il permet notamment de vérifier et valider si la distribution des données semble suivre la loi normale. Si la distribution des données sur le deuxième graphique semble ressembler davantage à une distribution normale, une transformation logarithmique des données pourrait être nécessaire. Cet histogramme illustre également l'occurrence moyenne par taxon par une ligne pleine ainsi que l'écart-type des occurrences par taxon par deux lignes pointillées. 
...
Pour chaque variable, le premier graphique (à gauche) illustre par un histogramme la distribution des fréquences des valeurs calculées de fréquences relatives en taxons à partir du fichier de données. Le deuxième graphique (à droite) illustre la distribution des fréquences des valeurs calculées de fréquences relatives en taxons à partir du fichier de données ayant subies une transformation logarithmique. Il permet notamment de vérifier et valider si la distribution des données semble suivre la loi normale. Si la distribution des données sur le deuxième graphique semble ressembler davantage à une distribution normale, une transformation logarithmique des données pourrait être nécessaire. Cet histogramme illustre également la fréquence relative moyenne par taxon par une ligne pleine ainsi que l'écart-type de la fréquence relative observée par taxon par deux lignes pointillées. 
...
Pour chaque variable, la première boîte à moustaches (à gauche) illustre la distribution des occurrences calculées à partir du fichier de données et la deuxième boîte à moustaches (à droite) illustre la distribution des occurrences calculées à partir du fichier de données ayant subies une transformation logarithmique.
...
Pour chaque variable, la première boîte à moustaches (à gauche) illustre la distribution des fréquences relatives calculées à partir du fichier de données et la deuxième boîte à moustaches (à droite) illustre la distribution des fréquences relatives calculées à partir du fichier de données ayant subies une transformation logarithmique.
...
###Matrice de corrélation entre les taxons - taxons avec fréquence > 25%
...
###Tests de corrélation de Pearson entre les taxons - taxons avec fréquence > 25%

Les résultats suivants indiquent la valeur de corrélation de Pearson entre des taxons. Une valeur de corrélation (cor) supérieure à 0.70 avec une valeur de P (p-value) inférieure à 0.05 indique qu'il est possible de supposer qu'une forte corrélation existe entre les variables avec une probabilité de 95%. Seulement les taxons présents dans plus de 25% des visites y sont représentés. 

```{r, echo=FALSE}
#Tests de corrélation de Pearson entre les taxons - taxons avec fréquence > 25%

options(max.print=1000000)
cor.matrix(Benthos25)
...
###Test de Student - taxons avec fréquence > 25%

Les résultats suivants présentent les résultats du test Fisher (comparaison de variances) appliqué entre les taxons. Une valeur de P (p-value) inférieure à 0.05 indique que les variances des deux taxons comparées dans le test ne sont pas égales. Le cas échéant, l'argument "var.equal = FALSE" écrit par défault doit être maintenu tel quel à la commande du test de Student suivant. Dans le cas contraire, l'argument "var.equal = TRUE" doit être remplacé par l'argument écrit par défault à la commande suivante. 

```{r, echo=FALSE}
#Test de Fisher : Comparaison de variances - taxons avec fréquence > 25%

{print(paste("F-Test : Comparaison de variances"))
for(i in 1:ncol(Benthos25)){
  for(j in 1:ncol(Benthos25)) {
var.test <- var.test(as.matrix(Benthos25[,i]), as.matrix(Benthos25[,j]))
print(paste(colnames(Benthos25[i]), "avec", paste(colnames(Benthos25[j]))))
print(var.test)
  }}}

```

Les résultats suivants présentent les résultats du test de Student (comparaison de moyennes) appliqué entre les taxons. Une valeur de P (p-value) inférieure à 0.05 indique que les moyennes des deux taxons comparées dans le test ne sont pas égales. Seulement les taxons présents dans plus de 25% des visites y sont représentés. Ce test n'est valide que si les suppositions de base suivantes sont répondues :  
- la variable dépendante suit une distribution normale ;  
- la variable dépendante est continue ;  
- présence d'une variable indépendante contenant deux groupes indépendants ;  
- indépendance entre les paires d'observations comparées.

```{r, echo=FALSE}

#Test de Student : Comparaison de moyennes - taxons avec fréquence > 25%

{print(paste("t-Test : Comparaison de moyennes"))
for(i in 1:ncol(Benthos25)){
  for(j in 1:ncol(Benthos25)) {
t.test <- t.test(Benthos25[,i], Benthos25[,j], var.equal = FALSE)
print(paste(colnames(Benthos25[i]), "avec", paste(colnames(Benthos25[j]))))
print(t.test)
  }}}

#Une valeur de P (p-value) inférieure à 0.05 indique que les deux variables comparées dans le test ne sont pas égales.

```


###Test de Wilcoxon Signed-Rank - taxons avec fréquence > 25%

Les résultats suivants présentent les résultats du test de Wilcoxon Signed-Rank appliqué entre les taxons. Une valeur de P (p-value) inférieure à 0.05 indique que les deux taxons comparés dans le test ne sont pas identiques. Seulement les taxons présents dans plus de 25% des visites y sont représentés. Ce test n'est valide que si les suppositions de base suivantes sont répondues :
- présence d'une variable dépendante continue ou ordinale ;  
- présence d'une variable indépendante contenant deux groupes catégoriques appariés ;  
- indépendance entre les paires d'observations comparées ;  
- les différences de distributions des deux groupes de la variable indépendante sont symétriques.
...
for(i in 1:ncol(Benthos25)) {
  for(j in 1:ncol(Benthos25)) {
wilcox.test <- wilcox.test(Benthos25[,i], Benthos25[,j], paired = TRUE)
print(paste(colnames(Benthos25[i]), "avec", paste(colnames(Benthos25[j]))))
print(wilcox.test)
  }}
...
###Test de Mann-Whitney-Wilcoxon - taxons avec fréquence > 25%

Les résultats suivants présentent les résultats du test de Mann-Whitney-Wilcoxon appliqué entre les taxons. Une valeur de P (p-value) inférieure à 0.05 indique que les deux taxons comparés dans le test ne sont pas identiques. Seulement les taxons présents dans plus de 25% des visites y sont représentés. Ce test n'est valide que si les suppositions de base suivantes sont répondues :
- présence d'une variable dépendante continue ou ordinale ;   
- présence d'une variable indépendante contenant deux groupes indépendants catégoriques ;  
- indépendance entre les observations ;  
- les distributions des groupes de la variable indépendante ont la même variabilité.
...
for(i in 1:ncol(Benthos25)) {
  for(j in 1:ncol(Benthos25)) {
wilcox.test <- wilcox.test(Benthos25[,i], Benthos25[,j], data = Benthos25, paired = F)
print(paste(colnames(Benthos25[i]), "avec", paste(colnames(Benthos25[j]))))
print(wilcox.test)
  }}
...
###Test de Kruskal-Wallis - taxons avec fréquence > 25%

Les résultats suivants présentent les résultats du test de Kruskal-Wallis appliqué entre les taxons. Une valeur de P (p-value) inférieure à 0.05 indique que les deux taxons comparés dans le test ne sont pas identiques. Seulement les taxons présents dans plus de 25% des visites y sont représentés. Ce test n'est valide que si les suppositions de base suivantes sont répondues :
- présence d'une variable dépendante continue ou ordinale ;  
- présence d'une variable indépendante contenant deux groupes indépendants catégoriques et plus ;  
- indépendance entre les observations ;  
- les distributions des groupes de la variable indépendante ont la même variabilité.
...
for(i in 1:ncol(Benthos25)) {
  for(j in 1:ncol(Benthos25)) {
kruskal.test <- kruskal.test(Benthos25[,i], Benthos25[,j])
print(paste(colnames(Benthos25[i]), "avec", paste(colnames(Benthos25[j]))))
print(kruskal.test)
  }}
...
###Graphique de dispersion entre l'abondance moyenne par taxon et l'occurrence par taxon

Le graphique suivant illustre la relation entre l'abondance moyenne par taxon et l'occurrence par taxon. 

```{r, echo=FALSE}
#Graphique de dispersion entre l'abondance moyenne par taxon et l'occurrence par taxon

abundance.mean.sp <- colMeans(dataset.BIO, na.rm = TRUE)

qplot(as.numeric(spe.pres), as.numeric(abundance.mean.sp), xlab = "Occurrence", ylab = "Abondance moyenne", main = "Relation entre l'abondance moyenne par taxon et l'occurrence par taxon") +
  geom_text(aes(label = ifelse(as.numeric(spe.pres) > 55 | as.numeric(abundance.mean.sp) > 100, paste(names(dataset.BIO)), "")), size = 3, hjust = 1, vjust = -0.75, nudge_x = -1) +
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_smooth(aes(colour="Intervalle de confiance de\n95% du modèle")) +
  scale_colour_manual(name="Légende", values=c("blue"))

```


###Graphique de dispersion entre l'abondance totale en individus et la richesse en taxons par visite

Les graphiques suivants illustrent la relation entre l'abondance totale en individus et la richesse en taxons par site.

```{r, echo=FALSE}
#Graphiques de dispersion entre l'abondance totale en individus et la richesse en taxons par visite

richness <- apply(dataset.BIO > 0, 1, sum, na.rm=TRUE)

{par(mfrow = c(1, 2), mar = c(4,4,4,1))
plot(sort(richness), xlab = "Ordre des visites", ylab = "Richesse en taxons", main = "Ordre croissant de la richesse\nen taxonspar visite")
plot(sort(richness), log = 'y', xlab = "Ordre des visites", ylab = "Log(Richesse en taxons)", main = "Ordre croissant du logarithme\nde la richesse en taxons\npar visite")

spe.ab <- apply(dataset.BIO, 1, sum, na.rm = T)
plot(sort(spe.ab), xlab = "Ordre des visites", ylab = "Abondance totale en taxons", main = "Ordre croissant de l'abondance\ntotale en taxons par visite")
plot(sort(spe.ab), log = 'y', xlab = "Ordre des visites", ylab = "Log(Abondance totale en taxons)", main = "Ordre croissant du logarithme\nde l'abondance totale en taxons\npar visite")

plot(richness, spe.ab, xlab = "Richesse en taxons", ylab = "Abondance totale en taxons", main = "Relation entre l'abondance\ntotale en taxons et la richesse\nen taxons par visite") +
  abline(lsfit(richness, spe.ab),col="red")
}
...
for(i in 2:(ncol(dataset.BIO2)-5)){
  ...
  graph.time <- plot(dataset.BIO2$SampleDate, dataset.BIO2[,i], xlab = "Temps  (année)", ylab = "Abondance", main = paste("Abondance observée de\n", colnames(dataset.BIO2[i]), "dans le temps"))

  graph.time2 <- ts(dataset.BIO2[,i], frequency = 12, start = c(2005-01-01), end = c(2016-01-01))
  plot(graph.time2, xlab = "Temps (année)", ylab = "Abondance", main = paste("Prévisions de l'abondance de\n", colnames(dataset.BIO2[i]), "dans le temps"))
...
Les résultats suivants présentent les résultats du test de la statistique de Ljung–Box appliqué sur chaque variable et viennent complémenter et confirmer les résultats illustrés par les graphiques de la commande précédente. Une valeur de P (p-value) inférieure à 0.05 permet de supposer que les valeurs résiduelles d'une variable dépendent de la période de l'année (temps).
...
Le tableau suivant présente différents indices de diversité en taxons calculés pour chaque site. 
...
Pour chaque variable, le premier graphique (à gauche) illustre par un histogramme la distribution des fréquences des valeurs calculées d'occurrence en diversité de Shannon à partir du fichier de données. Le deuxième graphique (à droite) illustre la distribution des fréquences des valeurs d'occurrence en diversité de Shannon calculées à partir du fichier de données ayant subies une transformation logarithmique. Il permet notamment de vérifier et valider si la distribution des données semble suivre la loi normale. Si la distribution des données sur le deuxième graphique semble ressembler davantage à une distribution normale, une transformation logarithmique des données pourrait être nécessaire. Cet histogramme illustre également l'occurrence moyenne par taxon par une ligne pleine ainsi que l'écart-type des occurrences par taxon par deux lignes pointillées. 
...
Pour chaque variable, le premier graphique (à gauche) illustre par un histogramme la distribution des fréquences des valeurs calculées d'occurrence en diversité de Simpson à partir du fichier de données. Le deuxième graphique (à droite) illustre la distribution des fréquences des valeurs d'occurrence en diversité de Simpson calculées à partir du fichier de données ayant subies une transformation logarithmique. Il permet notamment de vérifier et valider si la distribution des données semble suivre la loi normale. Si la distribution des données sur le deuxième graphique semble ressembler davantage à une distribution normale, une transformation logarithmique des données pourrait être nécessaire. Cet histogramme illustre également l'occurrence moyenne par taxon par une ligne pleine ainsi que l'écart-type des occurrences par taxon par deux lignes pointillées. 
...
###Boîtes à moustaches de l'occurrence de la diversité de Shannon

Pour chaque variable, la première boîte à moustaches (à gauche) illustre la distribution des occurrences de la diversité de Shannon calculées à partir du fichier de données et la deuxième boîte à moustaches (à droite) illustre la distribution des fréquences relatives calculées à partir du fichier de données ayant subies une transformation logarithmique.

```{r, echo=FALSE}
#Boîtes à moustaches de l'occurrence de la diversité de Shannon

box.N1.1 <- qplot(y = N1, x = "", geom = "boxplot", ylab = "Diversité de Shannon", main = "Distribution des occurrences de la diversité de \nShannon") + 
  theme(plot.title = element_text(hjust = 0.5))
box.N1.2 <- qplot(y = N1, x = "", geom = "boxplot", ylab = "Log(Diversité de Shannon)", main = "Distribution des occurrences de la diversité de \nShannon", log = "y") + 
  theme(plot.title = element_text(hjust = 0.5))

grid.arrange(box.N1.1, box.N1.2, ncol = 2)

```


###Boîtes à moustaches de l'occurrence de la diversité de Simpson

Pour chaque variable, la première boîte à moustaches (à gauche) illustre la distribution des occurrences de la diversité de Simpson calculées à partir du fichier de données et la deuxième boîte à moustaches (à droite) illustre la distribution des fréquences relatives calculées à partir du fichier de données ayant subies une transformation logarithmique.

```{r, echo=FALSE}
#Boîtes à moustaches de l'occurrence de la diversité de Simpson

box.N2.1 <- qplot(y = N2, x = "", geom = "boxplot", ylab = "Diversité de Simpson", main = "Distribution des occurrences de la diversité de \nSimpson") + 
  theme(plot.title = element_text(hjust = 0.5))
box.N2.2 <- qplot(y = N2, x = "", geom = "boxplot", ylab = "Log(Diversité de Simpson)", main = "Distribution des occurrences de la diversité de \nSimpson", log = "y") + 
  theme(plot.title = element_text(hjust = 0.5))

grid.arrange(box.N2.1, box.N2.2, ncol = 2)
...
**RCBA_vv_biologique.Rmd Version 1.1 — 6 novembre 2017**

Cette version présente des suggestions d'actions en rapport avec la vérification et validation des données biologiques.

![ACTION:](../../../Configuration/logo_new.png)   **Actions**.

**RCBA_vv_biologique.Rmd Version 1.0 — 18 août 2017**

Version initiale de l'outil de vérification et validation des données biologiques.
