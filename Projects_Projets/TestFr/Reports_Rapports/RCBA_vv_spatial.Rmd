...
Ce rapport présente les résultats de la vérification et la validation des données géographiques pour le projet **`r #paste(datasetName)`**.
...
> 
> 
> - *La distribution géographique des sites présentent-elles des anomalies?*
...
leaflet(data = dataset.SPA2[, 2:3]) %>% addProviderTiles(providers$Esri.WorldTopoMap) %>%
  addAwesomeMarkers(~Longitude, ~Latitude, popup = paste("Station_Année :", dataset.SPA2$Site_Year), label = dataset.SPA2$Site_Year, icon = makeAwesomeIcon(icon = 'circle', markerColor = getColor1(dataset.SPA2)), clusterOptions = markerClusterOptions()) #%>% addLegend(
...
plot.ab <- ggplot(dataset.SPA2, aes(x = dataset.SPA2$Site_Date_Number, y = dataset.SPA2[,j])) +
  geom_point() +
  labs(y = colnames(dataset.SPA2[j]), x = "Visite") +
