...
if(!"Longitude" %in% colnames(dataset.ENV2)) {
...
if(!"Site" %in% colnames(dataset.SPA2)) {
...
if(!"Site_Date_Number" %in% colnames(dataset.ENV2)) {
...
box.ab.1 <- qplot(y = dataset.ENV2[,i], x = "", geom = "boxplot", ylab = colnames(dataset.ENV2[i])) + 
  theme(plot.title = element_text(hjust = 0.5)) +
  ggtitle(paste("Observed Values"))
box.ab.2 <- qplot(y = dataset.ENV2[,i], x = "", geom = "boxplot", ylab = paste("Log(", colnames(dataset.ENV2[i]),")"), log = "y") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  ggtitle(paste("Log Values"))

grid.arrange(box.ab.1, box.ab.2, ncol = 2, top = colnames(dataset.ENV2[i]))
}
...
for (j in 2:(ncol(dataset.ENV2)-3)) {
  ...
  grid.arrange(boxplot, dotchart, ncol = 2, top = colnames(dataset.ENV2[j]))
}
...
hist.ab.2 <- tryCatch(hist(log(as.numeric(unlist(dataset.ENV[,i]))), breaks = "Sturges", xlab = "Log(Value)", ylab = "Frequency", main = "Frequency Distribution (log)"), error=function(e) plot.new())
...
grid.arrange(box.ab.1, box.ab.2, ncol = 2, top = colnames(dataset.ENV[i]))
}
...
PlotMatrix(Env, nrows = 3, ncols = 3, panel = panel.smooth)
...
for(i in 2:(ncol(dataset.ENV2)-4)){
  ...
  graph.time2 <- ts(dataset.ENV2[,i], frequency = 12, start = c(2008-01-01), end = c(2013-01-01))
  graph.time22 <- plot(graph.time2, xlab = "Year", ylab = "Value", main = "Expected values", ylim = c(min(graph.time2), max(graph.time2)))

  auto.arima <- auto.arima(graph.time2)
  graph.auto.arima <- plot(forecast(auto.arima, h = 120), main = "Forecast (ARIMA)")
  
  graph.lag <- acf(graph.time2, na.action = na.pass, xlab = "Lag (year)", ylab = "ACF", main = "Autocorrelation Function", ylim = c(0, 1))
