...
# Create a unified list of samples (Site, date, et Sample number)
	names.BIO <- all.dataset.BIO[, c("Site", "SampleDate", "SampleId")]
	names.HAB <- all.dataset.ENV[, c("Site", "SampleDate", "SampleId")]
...
for(i in 1:nrow(NoMatch_Dataset)){for(j in 1:ncol(NoMatch_Dataset)){NoMatch_Dataset[i,j] <- length(setdiff(get(c("dataset.BIO2", "dataset.ENV2", "dataset.GEN", "dataset.NAM", "dataset.SPA")[i])$SampleId, get(c("dataset.BIO2", "dataset.ENV2", "dataset.GEN", "dataset.NAM", "dataset.SPA")[j])$SampleId))}}
