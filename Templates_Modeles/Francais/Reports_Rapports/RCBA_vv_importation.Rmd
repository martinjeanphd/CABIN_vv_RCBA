---
title: "Importation de données RCBA"
output:
  html_notebook:
    toc: yes
    toc_depth: 4
...
```{r, include=FALSE}
SEP <- svDialogs::dlgInput("Entrez le caractère de séparation utilisé entre les valeurs (par défaut, \";\"):", default = ";", Sys.info()["SEP"])$res
if(is.null(SEP) | length(SEP) == 0) {SEP = ";"} else {if(SEP == "NA") {
SEP <- ";"}}
DEC <- svDialogs::dlgInput("Entrez le caractère de marque de décimales (par défaut, \",\"):", default = ",", Sys.info()["DEC"])$res
...
# Demander à l'utilisateur si une valeur manquante est utilisée. Dans l'affirmative, la demander (par défaut: -99)

na.vals <- if(svDialogs::dlg_message("Vos données contiennent-elles une valeur indiquant des valeurs manquantes?", "yesno")$res == "yes"){svDialogs::dlgInput("Saisissez la valeur indiquant des valeurs manquantes:", default = "-99", Sys.info()["na.vals"])$res} else {""}
...
# %a: Nom abbrégé du jour en caractères (selon la langue de votre système)

dateformat <- svDialogs::dlgInput("Saissisez le format de date:

%Y: Année avec le siècle (2017)
%y: Année sans le siècle (00-99)
%m: Mois avec deux chiffres (01-12)
%B: Mois en caractères (selon la langue de votre système)
%b: Mois abbrévié en caractères (selon la langue de votre système)
%d: Jours du mois avec deux chiffres (01-31)
%A: %A: Nom complet du jour en caractères (selon la langue de votre système)
Nom abbrégé du jour en caractères (selon la langue de votre système)
...
**La liste des taxons exclus *blacklist_taxa.csv* a été utilisée.**
...
envFilename <- tail(strsplit(svDialogs::dlgOpen(default = './../Data_Donnees/Habitat.csv', title="Sélectionnez le fichier contenant les données d'habitat:", multiple = FALSE, Sys.info()["envFilename"])$res, split="/")[[1]], n=1)
if(is.null(envFilename) | length(envFilename) == 0) {na.vals = "Habitat.csv"} else {if(envFilename == "NA") {
envFilename <- "Habitat.csv"}}

bioFilename <- tail(strsplit(svDialogs::dlgOpen(default = './../Data_Donnees/Benthic.csv', title="Sélectionnez le fichier contenant les données biologiques:", multiple = FALSE, Sys.info()["bioFilename"])$res, split="/")[[1]], n=1)
...
# Importer les données d'habitat et supprimer les lignes et colonnes vides
# Exécutez le code sans modification
...
#   RCBA, créer une liste unifiée des noms d'échantillons (Site, date, et numéro d'échantillon)
	...
	dataset.NAM2 <- arrange(dataset.NAM, Site, SampleDate, SampleNumber)
```
...
rm(dataset.SPAb)
rm(dataset.SPAe)
paste("Un tableau de coordonnées géographiques créé à partir d'un extrait des fichiers", bioFilename, "et", envFilename)
spaFilename <- paste("Extrait de ", bioFilename, "and", envFilename, sep="")
...
### Correspondance des jeux de données

Les différents jeux de données ont été comparés afin d'identifier des enregistrements manquants.

Les rangées représentent les enregistrements seulement présents dans ce jeu de données en comparaison avec la colonne correspondante.

```{r Dataset_Equity_Check, echo=FALSE}
NoMatch_Dataset <- matrix(ncol=5, nrow=5)
rownames(NoMatch_Dataset) <- c("Biologique", "Environmental", "Général", "Nom", "Spatial")
colnames(NoMatch_Dataset) <- c("Biologique", "Environmental", "Général", "Nom", "Spatial")
...
sink(file="./../Reports_Rapports/Manquants_Bio.txt")
...
sink()
sink(file="./../Reports_Rapports/Manquant_Env.txt")
...
sink()
sink(file="./../Reports_Rapports/Manquant_Gen.txt")
...
sink()
sink(file="./../Reports_Rapports/Manquant_Nam.txt")
...
sink()
sink(file="./../Reports_Rapports/Manquant_Spa.txt")
...
save(analysisTitle, file="../Data_Donnees/TitreAnalyse.RData")
```
