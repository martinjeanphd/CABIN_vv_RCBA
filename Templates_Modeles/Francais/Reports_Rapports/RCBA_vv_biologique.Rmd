---
title: "Vérification et validation RCBA - données biologiques"
date: '2018-02-14'
output:
  html_notebook:
    df_print: paged
...
Ce rapport présente les résultats de la vérification et la validation des données biologiques pour le projet **`r paste(analysisTitle)`**.
...
## Create a folder, if needed, for saving results
 ifelse(dir.exists("Résultats_biologie"),"",dir.create("Résultats_biologie"))
...
> *Examinez les points suivants :*
> 
> - *La liste ci-haut correspond-t'elle avec le tableau suivant qui présente les visites présentes dans l'un ou l'autre des fichiers de données du projet `r paste(analysisTitle)`?*
...
cat("Nombre de données aberrantes potentielles:", length(Active_Frame_Bio[Active_Frame_Bio$"Site, Date and Number" != "",][[1]]), "\n\n")
}
...
colnames(Active_Frame_Bio) <- c("Site, Date and Number", "Is NA")
write.table(Active_Frame_Bio[Active_Frame_Bio$"Site, Date and Number" != "",], row.names = FALSE, quote = FALSE, na = "NA", sep = ", ")
cat("Nombre de valeurs averrantes potentielles:", length(Active_Frame_Bio[Active_Frame_Bio$"Site, Date and Number" != "",][[1]]), "\n\n")
}
...
Dans les études écologiques, nous devons réfléchir à ce que cela signifie lorsque deux espèces sont conjointement absentes. Cela pourrait nous renseigner sur les caractéristiques écologiques d'un site. Lorsque deux sites ont les mêmes absences conjointes, cela pourrait signifier que les sites sont écologiquement similaires. D'autre part, si une espèce a une distribution très agglomérée, ou est simplement rare, alors les absences conjointes peuvent survenir par hasard et ne rien dire de l'adéquation d'un site donné pour une espèce, de la similarité entre les besoins en habitat des espèces ou de la similarité écologique des sites. Une fréquence élevée de zéros peut donc compliquer considérablement l'interprétation de ces analyses.

### Histogramme de fréquences d'abondances

Les valeurs d'abondance d'un taxon présente généralement une distribution asymétrique qui ressemble à une distribution log-normale, avec de nombreuses valeurs petites à modérées et quelques valeurs extrêmement grandes.

L'histogramme suivant illustre la distribution des fréquences d'abondance des taxons. Ce graphique permet, entre autres, d'évaluer le nombre de valeurs 0 dans l'ensemble de données. 
...
Nous devons savoir s'il y a des doubles zéros dans les données. Cela signifie que pour chaque paire d'espèces, nous devons calculer combien de fois les deux ont eu une abondance nulle pour le même site.

La matrice de corrélation suivante (ou *corrgramme*) illustre la fréquence à laquelle les paires de taza ont toutes deux une abondance nulle. La couleur et la quantité de remplissage d'un cercle correspondent à la proportion d'observations comportant des doubles zéros. Une couleur blanche indique une corrélation proche de 0,5 tandis qu'une couleur bleu foncé indique une corrélation proche de 1. Cette matrice illustre également la proportion des valeurs de corrélation. 0 d'un taxon par rapport à l'ensemble des valeurs d'abondance observées dans ce dernier. La diagonale allant du bas à gauche au haut à droite représente le pourcentage d'observations d'un taxon égal à zéro. Seuls les taxons présents dans plus de 25% des visites sont représentés.

> ![ACTION:](../../../Configuration/action.png)
> *Examiner les points suivants:*
> 
> 
> - *Les données présentent-elles un gradient biologique court ou long ?
> - *Que signifie la présence de doubles zéros pour les données ?
...
Diverses techniques statistiques supposent la normalité. Il est important de savoir si la technique statistique à utiliser suppose effectivement la normalité, et que suppose-t-on exactement comme étant normalement distribué? Par exemple, une *analyse en composantes principales* (ACP) n'exige pas de normalité. La *Régression linéaire* suppose effectivement la normalité, mais est raisonnablement robuste contre la violation de vet exigence. Pour d'autres techniques comme l'*analyse discriminante*, la normalité des observations d'une variable particulière au sein de chaque groupe est importante. C'est pourquoi il est toujours recommandé de tester la normalité (entre autres choses) au début de la phase d'analyse des données d'un projet écologique.
...
for (i in 1:ncol(dataset.BIO)){
norm.mean <- mean(dataset.BIO[,i], na.rm = TRUE)
norm.sd <- sd(dataset.BIO[,i], na.rm = TRUE)
norm.n <- length(which(!is.na(dataset.BIO[,i])))
norm.bin <- ceiling(max(dataset.BIO[,i], na.rm = TRUE) - min(dataset.BIO[,i], na.rm = TRUE)/nclass.Sturges(which(!is.na(dataset.BIO[,i]))))
...
Une façon d'examiner la normalité des données est d'utiliser le test de Snows. Pour chaque taxon, ce calcul teste l'hypothèse nulle selon laquelle les données proviennent d'une population normale exacte.

C'est une hypothèse nulle beaucoup moins intéressante que ce que nous voulons habituellement, c'est-à-dire savoir si les données proviennent d'une distribution suffisamment similaire à la normale pour utiliser l'inférence de la théorie normale.

Une valeur de P (valeur p) inférieure à 0,05 indique qu'il n'est pas possible de supposer que la distribution des données suit la distribution normale avec une probabilité de 95%. 
...
div <- data.frame(N0, round(H, 2), round(N1, 2), round(N2, 2), round(J, 2), round(E10, 2), round(E20, 2), dataset.BIO2$Site_Date_Number)
